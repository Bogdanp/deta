...
(define sql-tests
  (test-suite
   ...
   (test-suite
    ...
    (check-emitted (select (concat "hello " "world!")) "SELECT CONCAT('hello ', 'world!')")
    ...
    (check-emitted (select (between (now)
                                    (- (now) (interval "7 days"))
                                    (+ (now) (interval "7 days"))))
                   "SELECT NOW() BETWEEN NOW() - INTERVAL '7 days' AND NOW() + INTERVAL '7 days'"))))
